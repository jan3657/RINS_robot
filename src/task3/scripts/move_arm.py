...
class Arm_Mover():
    def __init__(self):
        ...
        self.lift = JointTrajectory()
        self.lift.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint",
                                   "arm_wrist_flex_joint"]
        self.lift.points = [JointTrajectoryPoint(positions=[0, 0.1, 1, 0],
                                                   time_from_start=rospy.Duration(1))]
        ...
        self.left.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint",
                                  "arm_wrist_flex_joint"]
        self.left.points = [JointTrajectoryPoint(positions=[1.57, 0.3, 1, 0],
                                                  time_from_start=rospy.Duration(1))]

        self.back = JointTrajectory()
        self.back.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint",
                                 "arm_wrist_flex_joint"]
        self.back.points = [JointTrajectoryPoint(positions=[3, 0.3, 1, 0],
                                                 time_from_start=rospy.Duration(1))]

        self.front = JointTrajectory()
        self.front.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint",
                                 "arm_wrist_flex_joint"]
        self.front.points = [JointTrajectoryPoint(positions=[-3.5, 0.3, 1, 0],
                                                 time_from_start=rospy.Duration(1))]
    ...
    def update_position(self):
        ...
        if self.send_command:
            if self.user_command == 'retract':
                ...
                print('Back-ed arm!')
            elif self.user_command == 'front':
                self.arm_movement_pub.publish(self.front)
                print('Back-ed arm!')
            elif self.user_command == 'lift':
                self.arm_movement_pub.publish(self.lift)
                print('Lift-ed arm!')
            else:
